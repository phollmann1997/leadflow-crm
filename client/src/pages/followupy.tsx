...
import type { Followup, Firma } from "@shared/schema";
  ...
  const renderList = (items: Followup[]) => (
    ...
    <div className="space-y-2">
      {items.map(fu => {
        ...
        return (
          <div key={fu.id} className={`flex items-center gap-3 p-3 rounded-md border ${overdue ? "border-red-300 dark:border-red-800 bg-red-50/50 dark:bg-red-950/20" : "border-border/50"} ${fu.splneno ? "opacity-50" : ""}`}>
            ...
            <div className="flex items-center gap-1 shrink-0">
              ...
              <Button variant="ghost" size="icon" onClick={() => deleteMutation.mutate(fu.id)}><Trash2 className="h-3.5 w-3.5 text-muted-foreground" /></Button>
            </div>
          </div>
        );
      })}
  ...
  return (
    <div className="p-4 md:p-6 space-y-4 overflow-auto h-full">
      ...
      <Tabs defaultValue="aktivni">
        <TabsList>
          <TabsTrigger value="aktivni">K vyřízení ({nesplnene.length})</TabsTrigger>
          <TabsTrigger value="splnene">Splněné ({splnene.length})</TabsTrigger>
        </TabsList>
